 ...
 * @return array
 **/
function hpo_get_media_background_picture_srcs( $attachment_xs_id, $attachment_sm_id, $img_size_prefix ) {
	$bg_images = array();
	...
	if ( $attachment_sm_id ) {
		$bg_images = array_merge(
			$bg_images,
			array(
				'xl' => hpo_get_attachment_src_by_size( $attachment_sm_id, $img_size_prefix . '-xl' ),
				'lg' => hpo_get_attachment_src_by_size( $attachment_sm_id, $img_size_prefix . '-lg' ),
				'md' => hpo_get_attachment_src_by_size( $attachment_sm_id, $img_size_prefix . '-md' ),
				'sm' => hpo_get_attachment_src_by_size( $attachment_sm_id, $img_size_prefix . '-sm' )
			)
		);
		...
		if ( !$attachment_xs_id ) {
			$bg_images = array_merge(
				$bg_images,
				array( 'xs' => hpo_get_attachment_src_by_size( $attachment_sm_id, $img_size_prefix ) )
			);
		}
	...
	if ( $attachment_xs_id ) {
		$bg_images = array_merge(
			$bg_images,
			array( 'xs' => hpo_get_attachment_src_by_size( $attachment_xs_id, $img_size_prefix ) )
		);
	}
	...
	$bg_images = (array) apply_filters( 'hpo_get_media_background_picture_srcs', $bg_images );
 ...
 * @return string
 **/
if ( ! function_exists( 'hpo_get_media_background_picture' ) ) {
	function hpo_get_media_background_picture( $srcs ) {
		// NOTE: if a child theme overrides the `hpo_media_background_picture_object_position`
		// hook, that theme must also provide a style override to
		// `.media-background` to set the object-position property
		$object_position = apply_filters( 'hpo_media_background_picture_object_position', '' );
 ...
 * @return string
 **/
if ( ! function_exists( 'hpo_get_media_background_video' ) ) {
	function hpo_get_media_background_video( $videos, $loop=false ) {
		// NOTE: if a child theme overrides the `hpo_media_background_video_object_position`
		// hook, that theme must also provide a style override to
		// `.media-background-video` to set the object-position property
		$object_position = apply_filters( 'hpo_media_background_video_object_position', '' );
		$object_position_attr = ( ! empty( $object_position ) ) ? 'data-object-position="' . esc_attr( $object_position ) . '"' : '';
